...
/**
 * Write docs/dossier-tallies.json and docs/Tennessee_Flock_Index.json
 * from the three dossier JSON files plus docs/Tennessee_Flock_Extras.json.
...
var payload = tallies.computeAllTallies(misuse, cancel, wrongful);
payload.source_files = {
  ...
  tennessee_index: "Tennessee_Flock_Index.json",
};

fs.writeFileSync(out, JSON.stringify(payload, null, 2) + "\n");
process.stdout.write("Wrote " + path.relative(root, out) + "\n");

var tnIndex = require("../assets/tennessee-index.js");
var extrasPath = path.join(docs, "Tennessee_Flock_Extras.json");
var extras = { records: [] };
if (fs.existsSync(extrasPath)) {
  extras = JSON.parse(fs.readFileSync(extrasPath, "utf8"));
}
var indexPayload = tnIndex.buildTennesseeIndex(misuse, cancel, wrongful, extras, {
  as_of: payload.as_of,
  as_of_long: payload.as_of_long,
  as_of_short: payload.as_of_short,
});
var indexOut = path.join(docs, "Tennessee_Flock_Index.json");
fs.writeFileSync(indexOut, JSON.stringify(indexPayload, null, 2) + "\n");
process.stdout.write("Wrote " + path.relative(root, indexOut) + "\n");
