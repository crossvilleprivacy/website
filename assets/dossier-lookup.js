 ...
 * tennessee.html widgets lock to TN with data-state-lock).
...
(function (root, factory) {
  ...
  function bindLookup(doc, spec) {
    ...
    function draw() {
      var filtered = sortRows(
        spec.filter(
          records,
          search ? search.value : "",
          stateLock || (stateSelect ? stateSelect.value : ""),
    ...
    if (clearBtn) {
      clearBtn.addEventListener("click", function () {
        ...
        if (stateSelect && !stateLock) stateSelect.value = "";
  ...
  function initMisuseLookup(doc, options) {
    ...
    return bindLookup(doc, {
      ...
      error: "Could not load this table. Try again on the live site.",
