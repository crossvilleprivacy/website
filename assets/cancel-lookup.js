...
(function (root, factory) {
  ...
  function initCancelLookup(doc, options) {
    ...
    function draw() {
      ...
      page = sliced.page;
      if (tbody) {
        renderTable(doc, tbody, sliced.rows, anchors, function (state) {
          setState(state);
          page = 1;
          draw();
        });
    ...
    var LOAD_ERROR =
      "Could not load the cancellations list. Serve the site over http://localhost (file:// cannot read the JSON).";
